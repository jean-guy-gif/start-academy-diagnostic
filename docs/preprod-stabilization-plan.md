...
> **✅ §7 CLOS — 2026-07-12.** Restauration bout-en-bout validée sur projet Supabase test isolé (`start-academy-restore-test-20260712`, ref `xyotsupfgvmyhmrnxvln`, supprimé post-run) : 33/33 critères verts, SHA-256 du PDF téléchargé via signed URL identique au SHA-256 du backup (`9d3be332…`), navigabilité diagnostic → session → document confirmée. Preuve datée dans `docs/restore-test-log.md`. Séquence rejouable par `bash scripts/restore-test.sh`. Frictions techniques du run (mot de passe à caractères spéciaux, IPv6 Docker, bug `PGURL`, service_role invalide, base non-vierge après échec) tracées en §11.2 T-12/T-13/T-14 comme dette non bloquante.
...
- [x] Restauration SQL OK (counts cohérents). — 20/20 tables métier, `auth.users` = 4.
- [x] Pas de table avec RLS désactivée. — `pg_tables where rowsecurity=false` = 0 dans `public`.
- [x] Au moins 1 objet Storage restauré et vérifié. — PDF 79 812 B ré-uploadé au `storage_path` exact, signed URL 60 s téléchargée, **SHA-256 identique octet pour octet** au backup.
- [x] Procédure documentée dans le registre interne (date, opérateur, durée totale). — `docs/restore-test-log.md`, run du 2026-07-12, opérateur Laurent.
...
| **T-8** | **Dette produit — pas d'action UI pour la transition de statut session (2026-07-12, découvert pendant smoke §4 étape 22).** Le workflow de `training_sessions.status` progresse `created → planned → validated → delivered`. L'UI dispose des transitions initiales (création, validation pédagogique) mais **aucune commande UI ne permet de basculer `validated → delivered`**. Statut atteint aujourd'hui par `UPDATE public.training_sessions SET status = 'delivered' WHERE id = …` en SQL éditeur, ce qu'a fait le smoke §4 étape 22 pour tester le flow post-training review. Confirmé par audit : `find src/app/api/sessions -name route.ts` ne remonte aucun handler `PATCH /api/sessions/[id]/status` (contrairement à `/api/diagnostics/[id]/status` qui existe). **BLOQUANT PILOTE conditionnel** — si le pilote va jusqu'au bout du parcours (formation dispensée + review post-formation par le dirigeant), un opérateur devra chaque fois passer par le SQL éditeur pour débloquer la session : friction incompatible avec un usage terrain. Si le pilote s'arrête à la validation pédagogique (pas de formation réellement dispensée), T-8 devient post-pilote. Suivi via `docs/backlog-post-stabilisation.md` §2.2. | ✅ **Corrigé (PR #17)** | Claude | Livré 2026-07-12 en même temps que T-8-BIS (garde `session.status ∈ {delivered, report_sent}` sur `post-training-review` + event dédié `post_training_review_modified_after_send` sur `report_sent`). Machine à états `src/lib/sessions/session-status-transitions.ts` (source unique, 9 AVANT + 7 RETOUR = 16 arêtes, contract test verrouille 100 combinaisons fail-si-supprimée). Service serveur `transitionSessionStatus` avec garde de nuance : `support_ready → delivered` exige `support_quality_reviews.status = 'validated'` (sinon 409 `support_not_validated`). Route `PATCH /api/sessions/[id]/status` alignée sur pattern `support-quality` (requireApiRole + assertCanAccessSession + Zod strict 10 valeurs + service + `createActivityLog({event_type:'session_status_changed', metadata:{from,to}})`). UI : bloc dans `session-detail-view.tsx` = bouton « Passer à l'étape suivante » (rail principal) + lien discret « Changer manuellement » (dropdown des retours arrière autorisés) + confirmation modale renforcée sur `delivered` et `report_sent` (irréversibles côté produit ; canal hotfix SQL délibérément inconfortable, admin sans override). Suppression au passage du code mort `updateTrainingSessionStatus` de `session-service.ts` (0 caller). Vérification manuelle 2026-07-12 : avancement via UI sans SQL, garde T-8-BIS 409 confirmée, transitions interdites absentes du dropdown. Tests : +22 (7 matrice + 6 route + 9 T-8-BIS), suite 95/95 verte. |
