...
| **T-10c** | ~~Décision produit : cockpit partagé, PAS de filtre sur `/api/activity/recent`.~~ **Reclassé T-10c-BIS** (voir ci-dessous). La règle documentée reste : le cockpit expose l'activité inter-commerciale de Start Academy à tout rôle interne (`admin`/`commercial`/`trainer`) — entraide et reprise de dossier volontaires. Voir commentaire d'en-tête de `src/app/api/activity/recent/route.ts`. | ✅ Décision documentée (posture, non-ticket) | — | Commentaire d'en-tête de la route explicitant la posture. |
| **T-10c-BIS** | **NOUVEAU FINDING (2026-07-09)** — la décision « cockpit partagé » de T-10c reposait sur l'hypothèse « pas de données sensibles dans activity_logs ». Vérification pendant PR #9 : la table contient un champ `event_description` (texte libre 500 chars) alimenté par `POST /api/activity/log` avec `eventType: "note_added"` (cf. `src/app/(app)/sessions/[id]/session-activity-journal.tsx:165`). Ce champ **PEUT contenir des notes internes sensibles** et remonte tel quel dans `activity/recent`. La posture actuelle expose donc les notes cross-commercial. | ⏳ Décision produit requise | à assigner | Trois options : (a) filtrer `event_description` de la réponse `activity/recent` (garde la posture cockpit partagée mais masque le contenu libre), (b) filtrer les événements `note_added` par `session_id` accessible à l'appelant, (c) accepter la fuite et documenter noir sur blanc dans le PRD. Ma reco : option (a) — masque le libellé libre sans casser la visibilité inter-commerciale des types d'événements. |
